...
class TestUserQuerySet:
    ...
    def test_specialists_with_opinions_count(self, queryset):
        _experts = UserFactory.create_batch(3, role=UserRoleType.FACT_CHECKER)
        specialists = UserFactory.create_batch(3, role=UserRoleType.SPECIALIST)
        specialists_opinions_counts = [
            len(ExpertOpinionFactory.create_batch(randint(1, 5), judge=specialist))
            for specialist in sorted(specialists, key=lambda x: x.id)
        ]

        result = queryset.specialists_with_opinions_count()

        assert_that(result).contains_only(*specialists)
        assert_that(result).extracting("verified", sort="id").is_equal_to(
            specialists_opinions_counts
        )
